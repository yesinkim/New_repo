...
class AttentionDecoder(nn.Module):
    ...
    def forward(
        ...
        output = torch.cat((output, context))
        # write this line if you want to apply tanh
...
class Seq2SeqWithAttention(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.max_sequence_size):
            ...
            enc_output_i, enc_hidden = self.encoder(
                enc_input=enc_input_i, enc_hidden=enc_hidden
            )
            encoder_output[:, i, :] = enc_output_i

        decoder_output = torch.zeros(
            dec_input.size(0), self.max_sequence_size, self.d_hidden
        )
        ...
        for i in range(self.max_sequence_size):
            ...
            dec_output_i, dec_hidden = self.decoder(
